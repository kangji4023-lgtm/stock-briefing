...
def run_job():
  ...
  # 1) 국내 주요 종목 분석 (yfinance 활용으로 안정성 확보)
  kr_tickers = {
      "삼성전자": "005930.KS",
      "SK하이닉스": "000660.KS",
      "삼성전기": "009150.KS",
  ...
  try:
    for name, ticker in kr_tickers.items():
      df = yf.download(
          ticker, period="3mo", interval="1d", progress=False
      )
    ...
    top_kr_data = sorted(top_kr_data, key=lambda x: x["change"], reverse=True)
  ...
  try:
    for t in us_tickers:
      df_u = yf.download(t, period="3mo", interval="1d", progress=False)
  ...
  # 3) 보유 종목 분석 (삼성전자, SK하이닉스, 삼성전기, SK스퀘어, 현대차)
  my_stock_names = ["삼성전자", "SK하이닉스", "삼성전기", "SK스퀘어", "현대차"]
  my_results = []
  try:
    for name in my_stock_names:
      ticker = kr_tickers[name]
      df_m = yf.download(ticker, period="3mo", interval="1d", progress=False)
  ...
  for i, item in enumerate(top_kr_data[:5], 1):
    msg += (
        ...
        f"   - 저항선: {item['high']:,.0f}원 / 지지선: {item['low']:,.0f}원\n"
  ...
  for stock_info in my_results:
    msg += (
        f"• {stock_info['name']}\n"
        f"  - 현재가: {stock_info['price']:,.0f}원\n"
        f"  - 손절가: {stock_info['stop']:,.0f}원 / 목표가: {stock_info['target']:,.0f}원\n"
