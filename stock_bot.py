...
def get_kr_data(ticker):
    try:
        ...
        start = (datetime.now(KST) - pd.Timedelta(days=420)).strftime("%Y%m%d")
...
def add_indicators(df):
    ...
    df["VOL_CHANGE"] = ((volume / df["VOL20"]) - 1) * 100
...
def analyze_stock(name, ticker, market):
    ...
    prev_price = clean_number(df["Close"].iloc[-2])
    change = 0
    if prev_price > 0:
    ...
    volume_change = clean_number(last["VOL_CHANGE"])
    golden_cross = bool(last["GOLDEN_CROSS"])
    trend_up = bool(last["TREND_UP"])

    obv_up = False
    if len(df) >= 20:
        obv_now = clean_number(df["OBV"].iloc[-1])
        obv_old = clean_number(df["OBV"].iloc[-20])
    ...
    if volume_change >= 20: score += 1
    if obv_up: score += 1
    if golden_cross: score += 1

    recent_high = clean_number(df["High"].tail(20).max())
    recent_low = clean_number(df["Low"].tail(20).min())
    ...
    expected_return = ((target / price) - 1) * 100

    foreign = 0
    institution = 0
    if market == "KR":
        foreign, institution = get_kr_supply(ticker)

    value = 0
    if "Value" in df.columns:
        value = clean_number(df["Value"].iloc[-1])
    elif market == "KR":
        value = price * clean_number(df["Volume"].iloc[-1])
...
def collect_kr():
    ...
    for name, ticker in KR_STOCKS.items():
        try:
            ...
            print("KR ANALYZE ERROR:", name, e)
    result.sort(key=lambda x: (x["score"], x["change"]), reverse=True)
    return result
...
def collect_us():
    ...
    for name, ticker in US_STOCKS.items():
        try:
            ...
            print("US ANALYZE ERROR:", name, e)
    result.sort(key=lambda x: (x["score"], x["change"]), reverse=True)
    return result


def get_golden(items):
    return [x["name"] for x in items if x["golden_cross"]]


def get_supply_top(items):
    if not items:
        return []
    return sorted(items, key=lambda x: x["foreign"] + x["institution"], reverse=True)[:5]


def sector_analysis(all_items):
    lookup = {x["name"]: x for x in all_items}
    result = []
    for sector, names in SECTORS.items():
        data = [lookup[name] for name in names if name in lookup]
        if not data:
            continue
        avg_change = np.mean([x["change"] for x in data])
        avg_score = np.mean([x["score"] for x in data])
        result.append({"sector": sector, "change": avg_change, "score": avg_score})
    result.sort(key=lambda x: (x["score"], x["change"]), reverse=True)
    return result
...
def stock_detail(x):
    ...
    return (
        f"📌 {x['name']} {x['change']:+.2f}%\n"
        f"점수 {x['score']}/10 | GC {gc}\n"
        f"RSI {x['rsi']:.1f} | MACD {macd} | OBV {obv}\n"
        f"거래량 {x['volume_change']:+.1f}%\n"
        f"20선 {fmt_price(x['ma20'])} | 60선 {fmt_price(x['ma60'])} | 120선 {fmt_price(x['ma120'])}\n"
        f"지지 {fmt_price(x['support'])} | 저항 {fmt_price(x['resistance'])}\n"
        f"기술적 목표 {fmt_price(x['target'])} | 리스크 기준 {fmt_price(x['stop'])}\n"
...
def market_summary(kospi, kosdaq, nasdaq, sp500, vix):
    values = []
    if kospi: values.append(f"KOSPI {kospi['change']:+.2f}%")
    if kosdaq: values.append(f"KOSDAQ {kosdaq['change']:+.2f}%")
    if nasdaq: values.append(f"NASDAQ {nasdaq['change']:+.2f}%")
    if sp500: values.append(f"S&P500 {sp500['change']:+.2f}%")
    if vix: values.append(f"VIX {vix['price']:.1f}")
    return " | ".join(values)
...
def build_briefing():
    ...
    lines.append("\n━━━━━━━━━━━━━━")
    lines.append("🇰🇷 국내시장")
    if kospi: lines.append(f"KOSPI {kospi['price']:,.2f} ({kospi['change']:+.2f}%)")
    if kosdaq: lines.append(f"KOSDAQ {kosdaq['price']:,.2f} ({kosdaq['change']:+.2f}%)")

    # 미국
    lines.append("\n━━━━━━━━━━━━━━")
    lines.append("🇺🇸 미국시장")
    if nasdaq: lines.append(f"NASDAQ {nasdaq['price']:,.2f} ({nasdaq['change']:+.2f}%)")
    if sp500: lines.append(f"S&P500 {sp500['price']:,.2f} ({sp500['change']:+.2f}%)")
    if dow: lines.append(f"DOW {dow['price']:,.2f} ({dow['change']:+.2f}%)")
    ...
    for i, x in enumerate(supply, 1):
        lines.append(f"{i}. {x['name']} | 외국인 {fmt_money(x['foreign'])} | 기관 {fmt_money(x['institution'])}")
    ...
    # 오늘 모멘텀 후보
    momentum = sorted(kr + us, key=lambda x: (x["score"], x["volume_change"], x["change"]), reverse=True)
    lines.append("\n━━━━━━━━━━━━━━")
    lines.append("🚀 오늘 모멘텀 후보 TOP5")
    lines.append("※ 미래 상승확률을 임의로 만들지 않고 기술적 조건 충족도를 표시합니다.")
    ...
    # 투자 아이디어
    lines.append("\n━━━━━━━━━━━━━━")
    lines.append("💡 오늘 투자 아이디어 5가지")
    lines.append("① 국내 TOP 종목 중 20·60·120일선 정배열 여부 확인")
    lines.append("② 골든크로스 발생과 거래량 증가가 동시에 나타나는 종목 확인")
    lines.append("③ RSI 70 이상 종목은 추격매수보다 눌림 여부 확인")
    lines.append("④ VIX·미국10년물·환율 상승 여부를 동시에 확인")
    lines.append("⑤ 뉴스 발생 후 실제 거래량과 수급이 따라오는지 확인")
    ...
    if vix:
        if vix["price"] >= 25:
            lines.append("VIX 25 이상: 변동성 확대 구간 확인")
        elif vix["price"] >= 20:
    ...
    lines.append("\n━━━━━━━━━━━━━━")
    lines.append("📌 마지막 한줄")
    lines.append("데이터·뉴스·수급·기술적 지표가 동시에 개선되는 종목을 우선 관찰하고, 목표가와 리스크 기준을 함께 관리하세요.")
...
def main():
    print("========================================")
    print("AI 국내·미국 주식 브리핑 시작")
    KST = ZoneInfo("Asia/Seoul")
    now = datetime.now(KST)
    print(f"한국시간: {now.strftime('%Y-%m-%d %H:%M:%S')}")
    print("========================================")
    ...
    for i, message in enumerate(messages, 1):
        print(f"카카오 메시지 {i}/{len(messages)} 전송")
        send_kakao(message)
        time.sleep(2)
