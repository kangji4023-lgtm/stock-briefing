...
def get_naver_news(query, count=5):
    ...
    try:
        ...
        if response.status_code != 200:
            print("NAVER NEWS STATUS:", response.status_code)
            return []
...
def get_kr_supply(ticker):
    ...
    try:
        ...
        # 💡 Length mismatch 에러 방지를 위한 안전장치 추가
...
def make_move_reason(x):
    ...
    if volume_change >= 50:
        ...
        reasons.append(
            f"거래량 {volume_change:+.1f}% 급증"
        )

    elif volume_change >= 20:

        reasons.append(
            f"거래량 {volume_change:+.1f}% 증가"
        )

    elif volume_change <= -30:

        reasons.append(
            f"거래량 {volume_change:+.1f}% 감소"
        )
    ...
    if x["market"] == "KR":
        ...
        if foreign > 0:
            ...
            reasons.append(
                f"외국인 순매수 {fmt_money(foreign)}"
            )

        elif foreign < 0:

            reasons.append(
                f"외국인 순매도 {fmt_money(abs(foreign))}"
            )

        if institution > 0:

            reasons.append(
                f"기관 순매수 {fmt_money(institution)}"
            )

        elif institution < 0:

            reasons.append(
                f"기관 순매도 {fmt_money(abs(institution))}"
            )
...
def build_briefing():
    ...
    lines.append(
        "① 20·60·120일선 정배열 여부 확인"
    )

    lines.append(
        "② 골든크로스와 거래량 증가 동시 발생 여부"
    )
    ...
    lines.append(
        "환율·금리·유가·지정학 뉴스와 실제 주가 반응을 함께 확인하세요."
    )
    ...
    lines.append(
        "트럼프 발언·관세·무역정책·미중관계·금리·지정학 이슈는 관련 뉴스와 시장 반응을 함께 확인합니다."
    )
    ...
    lines.append(
        "주가 방향 + 거래량 + RSI + MACD + OBV + 이동평균 + 골든크로스 + 국내 수급 + 관련 뉴스를 함께 표시합니다."
    )
